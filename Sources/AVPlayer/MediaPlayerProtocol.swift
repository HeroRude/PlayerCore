...
public protocol MediaPlayerProtocol: MediaPlayback {
    ...
    @available(tvOS 14.0, *)
    var pipController: MoonPictureInPictureController? { get }
    var dynamicInfo: DynamicInfo? { get }
    init(url: URL, options: MoonOptions)
    func replace(url: URL, options: MoonOptions)
    func play()
    func pause()
...
public extension MediaPlayerTrack {
    ...
    var colorSpace: CGColorSpace? {
        MoonOptions.colorSpace(ycbcrMatrix: yCbCrMatrix as CFString?, transferFunction: transferFunction as CFString?)
    }
