 ...
 */
public enum MoonPlayerState: CustomStringConvertible {
    case prepareToPlay
    case readyToPlay
    case buffering
    case bufferFinished
...
public protocol MoonPlayerLayerDelegate: AnyObject {
    func player(layer: MoonPlayerLayer, state: MoonPlayerState)
    func player(layer: MoonPlayerLayer, currentTime: TimeInterval, totalTime: TimeInterval)
    func player(layer: MoonPlayerLayer, finish error: Error?)
    func player(layer: MoonPlayerLayer, bufferedCount: Int, consumeTime: TimeInterval)
}

open class MoonPlayerLayer: UIView {
    public weak var delegate: MoonPlayerLayerDelegate?
    @Published
    ...
    public private(set) var options: MoonOptions
    public var player: MediaPlayerProtocol {
        didSet {
            oldValue.view?.removeFromSuperview()
            UILog("player is \(player)")
    ...
    public private(set) var url: URL {
        didSet {
            ...
            if isWirelessRouteActive {
                ...
                firstPlayerType = MoonAVPlayer.self
            } else if options.display != .plane {
                // AR模式只能用KSMEPlayer
                // swiftlint:disable force_cast
                firstPlayerType = NSClassFromString("PlayerCore.MoonMEPlayer") as! MediaPlayerProtocol.Type
                // swiftlint:enable force_cast
            } else {
                firstPlayerType = MoonOptions.firstPlayerType
            }
    ...
    /// 播发器的几种状态
    public private(set) var state = MoonPlayerState.prepareToPlay {
        didSet {
            if state != oldValue {
                runInMainqueue { [weak self] in
                    ...
                    UILog("playerStateDidChange - \(self.state)")
    ...
    public init(url: URL, options: MoonOptions, delegate: MoonPlayerLayerDelegate? = nil) {
        self.url = url
    ...
    public func set(url: URL, options: MoonOptions) {
        self.options = options
        runInMainqueue {
    ...
    public func set(urls: [URL], options: MoonOptions) {
        self.options = options
        self.urls.removeAll()
    ...
    open func play() {
        ...
        if #available(tvOS 14.0, *) {
            MoonPictureInPictureController.mute()
        }
    }
...
extension MoonPlayerLayer: MediaPlayerDelegate {
    ...
    public func finish(player: some MediaPlayerProtocol, error: Error?) {
        if let error {
            if type(of: player) != MoonOptions.secondPlayerType, let secondPlayerType = MoonOptions.secondPlayerType {
                self.player = secondPlayerType.init(url: url, options: options)
                return
            }
            state = .error
            UILog(error as CustomStringConvertible)
        } else {
...
@available(tvOS 14.0, *)
extension MoonPlayerLayer: AVPictureInPictureControllerDelegate {
...
extension MoonPlayerLayer {
    ...
    @objc private func enterBackground() {
        ...
        if MoonOptions.canBackgroundPlay {
