...
public enum PlayerErrorCode: Int {
    case unknown
    case formatCreate
...
extension PlayerErrorCode: CustomStringConvertible {
    public var description: String {
        switch self {
        case .formatCreate:
...
extension NSError {
    convenience init(errorCode: PlayerErrorCode, userInfo: [String: Any] = [:]) {
        var userInfo = userInfo
        userInfo[NSLocalizedDescriptionKey] = errorCode.description
        self.init(domain: PlayerErrorDomain, code: errorCode.rawValue, userInfo: userInfo)
    }
}
...
#if !SWIFT_PACKAGE
extension Bundle {
    static let module = Bundle(for: MoonPlayerLayer.self).path(forResource: "KSPlayer_KSPlayer", ofType: "bundle").flatMap { Bundle(path: $0) } ?? Bundle.main
}
#endif
