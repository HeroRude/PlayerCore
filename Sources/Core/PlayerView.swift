...
public protocol PlayerControllerDelegate: AnyObject {
    func playerController(state: MoonPlayerState)
...
open class PlayerView: UIView, MoonPlayerLayerDelegate, KSSliderDelegate {
    ...
    public convenience init() {
        ...
        #else
        self.init(frame: CGRect(origin: .zero, size: MoonOptions.sceneSize))
        #endif
    }
    ...
    open func seek(time: TimeInterval, completion: @escaping ((Bool) -> Void)) {
        playerLayer?.seek(time: time, autoPlay: MoonOptions.isSeekedAutoPlay, completion: completion)
    }
    ...
    open func set(url: URL, options: MoonOptions) {
        srtControl.url = url
        toolBar.currentTime = 0
        totalTime = 0
        playerLayer = MoonPlayerLayer(url: url, options: options)
    }
    ...
    open func player(layer: MoonPlayerLayer, state: MoonPlayerState) {
    ...
    open func player(layer _: MoonPlayerLayer, currentTime: TimeInterval, totalTime: TimeInterval) {
    ...
    open func player(layer _: MoonPlayerLayer, finish error: Error?) {
        delegate?.playerController(finish: error)
    }

    open func player(layer _: MoonPlayerLayer, bufferedCount: Int, consumeTime: TimeInterval) {
