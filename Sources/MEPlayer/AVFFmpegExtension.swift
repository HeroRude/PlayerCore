...
extension AVCodecParameters {
    mutating func createContext(options: MoonOptions) throws -> UnsafeMutablePointer<AVCodecContext> {
...
extension AVChannelLayout: CustomStringConvertible {
    ...
    var layoutTag: AudioChannelLayoutTag? {
        UILog("[audio] FFmepg AVChannelLayout: \(self) order: \(order) mask: \(u.mask)")
        ...
        if let tag {
            ...
            UILog("[audio] can not find AudioChannelLayoutTag FFmepg channelLayout: \(self) order: \(order) mask: \(u.mask)")
            return nil
        }
    }
...
extension NSError {
    convenience init(errorCode: PlayerErrorCode, avErrorCode: Int32) {
