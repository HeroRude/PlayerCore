...
extension AudioUnit {
    var channelLayout: UnsafeMutablePointer<AudioChannelLayout> {
        ...
        UILog("[audio] unit tag: \(tag)")
        if tag == kAudioChannelLayoutTag_UseChannelDescriptions {
            UILog("[audio] unit channelDescriptions: \(layout.channelDescriptions)")
            return layout
        }
...
extension AVAudioChannelLayout {
    func channelLayout() -> AVChannelLayout {
        UILog("[audio] channelLayout: \(layout.pointee.description)")
        var mask: UInt64?
        if layoutTag == kAudioChannelLayoutTag_UseChannelDescriptions {
            ...
            layout.channelDescriptions.forEach { description in
                ...
                UILog("[audio] label: \(label)")
                let channel = label.avChannel.rawValue
                UILog("[audio] avChannel: \(channel)")
                if channel >= 0 {
                    newMask |= 1 << channel
                }
            }
        ...
        UILog("[audio] out mask: \(outChannel.u.mask) nb_channels: \(outChannel.nb_channels)")
        return outChannel
    }
