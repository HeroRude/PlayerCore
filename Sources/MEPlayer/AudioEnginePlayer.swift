...
//    private let delay = AVAudioUnitDelay()
    ...
    public func prepare(audioFormat: AVAudioFormat) {
        ...
        UILog("[audio] set preferredOutputNumberOfChannels: \(audioFormat.channelCount)")
        #endif
        UILog("[audio] outputFormat AudioFormat: \(audioFormat)")
        if let channelLayout = audioFormat.channelLayout {
            UILog("[audio] outputFormat channelLayout \(channelLayout.channelDescriptions)")
        }
        ...
        UILog("[audio] new sourceNode inputFormat: \(sourceNode.inputFormat(forBus: 0))")
