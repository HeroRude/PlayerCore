...
final class MEPlayerItem {
    private let url: URL
    private let options: MoonOptions
    ...
    private static var onceInitial: Void = {
        ...
        av_log_set_callback { ptr, level, format, args in
            ...
            if let ptr {
                ...
                if avclass == &ffurl_context_class {
                    ...
                    if let opaque = context.graph?.pointee.opaque {
                        let options = Unmanaged<MoonOptions>.fromOpaque(opaque).takeUnretainedValue()
            ...
            if log.hasPrefix("parser not found for codec") {
                UILog(level: .error, log)
            }
            UILog(level: LogLevel(rawValue: level) ?? .warning, log)
        }
    }()

    weak var delegate: MEPlayerDelegate?
    init(url: URL, options: MoonOptions) {
        self.url = url
...
extension MEPlayerItem {
    private func openThread() {
        ...
        if let protocolWhitelistCString = "file,crypto,data,http,https,tls,tcp,httpproxy".cString(using: .utf8) {
...
//        }
    ...
    private func openOutput(url: URL) {
        ...
        guard let outputFormatCtx, let formatCtx else {
            UILog(NSError(errorCode: .formatOutputCreate, avErrorCode: ret))
            return
        }
        var index = 0
        var audioIndex: Int?
        ...
        guard ret >= 0 else {
            UILog(NSError(errorCode: .formatWriteHeader, avErrorCode: ret))
    ...
    private func read() {
        readOperation = BlockOperation { [weak self] in
            ...
            Thread.current.stackSize = MoonOptions.stackSize
            self.readThread()
        }
...
//                var result = av_seek_frame(formatCtx, -1, timeStamp, seekFlags)
                ...
                if result < 0, seekFlags & AVSEEK_FLAG_BACKWARD == AVSEEK_FLAG_BACKWARD {
                    UILog("seek to \(seekToTime) failed. seekFlags remove BACKWARD")
                    options.seekFlags &= ~AVSEEK_FLAG_BACKWARD
                    seekFlags &= ~AVSEEK_FLAG_BACKWARD
                    result = avformat_seek_file(formatCtx, -1, seekMin, timeStamp, seekMax, seekFlags)
                }
                UILog("seek to \(seekToTime) spend Time: \(CACurrentMediaTime() - seekStartTime)")
                if state == .closed {
                    break
                }
        ...
        if readResult == 0 {
            if let outputFormatCtx, let formatCtx {
                   ...
                   let outputPacket
                {
                    ...
                    if ret < 0 {
                        UILog("can not av_interleaved_write_frame")
                    }
                }
            }
