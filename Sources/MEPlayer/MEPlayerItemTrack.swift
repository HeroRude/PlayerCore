...
protocol PlayerItemTrackProtocol: CapacityProtocol, AnyObject {
    init(mediaType: AVFoundation.AVMediaType, frameCapacity: UInt8, options: MoonOptions)
    // 是否无缝循环
...
class SyncPlayerItemTrack<Frame: MEFrame>: PlayerItemTrackProtocol, CustomStringConvertible {
    var seekTime = 0.0
    fileprivate let options: MoonOptions
    ...
    required init(mediaType: AVFoundation.AVMediaType, frameCapacity: UInt8, options: MoonOptions) {
...
//                    KSLog("seektime \(self.seekTime), frame \(frame.seconds), mediaType \(packet.assetTrack.mediaType)")
                ...
                UILog("Decoder did Failed : \(error)")
                if decoder is VideoToolboxDecode {
                    ...
                    UILog("VideoCodec switch to software decompression")
