...
public class MoonMEPlayer: NSObject {
    ...
    private lazy var _pipController: Any? = {
        if #available(iOS 15.0, tvOS 15.0, macOS 12.0, *), let videoOutput {
            ...
            let pip = MoonPictureInPictureController(contentSource: contentSource)
            return pip
        } else {
            return nil
        }
    ...
    @available(tvOS 14.0, *)
    public var pipController: MoonPictureInPictureController? {
        _pipController as? MoonPictureInPictureController
    }
    ...
    public required init(url: URL, options: MoonOptions) {
        MoonOptions.setAudioSession()
        audioOutput = MoonOptions.audioPlayerType.init()
        playerItem = MEPlayerItem(url: url, options: options)
        if options.videoDisable {
            videoOutput = nil
        } else {
            videoOutput = MoonOptions.videoPlayerType.init(options: options)
        }
...
private extension MoonMEPlayer {
    func playOrPause() {
    ...
    @objc private func spatialCapabilityChange(notification _: Notification) {
        UILog("[audio] spatialCapabilityChange")
...
extension MoonMEPlayer: MEPlayerDelegate {
    func sourceDidOpened() {
        ...
        runInMainqueue { [weak self] in
            ...
            if let audioDescriptor {
                UILog("[audio] audio type: \(self.audioOutput) prepare audioFormat )")
    ...
    func sourceDidChange(oldBitRate: Int64, newBitrate: Int64) {
        UILog("oldBitRate \(oldBitRate) change to newBitrate \(newBitrate)")
    }
}

extension MoonMEPlayer: MediaPlayerProtocol {
    ...
    public var naturalSize: CGSize {
        options.display == .plane ? playerItem.naturalSize : MoonOptions.sceneSize
    }
    ...
    public func replace(url: URL, options: MoonOptions) {
        UILog("replaceUrl \(self)")
        shutdown()
        ...
        if MoonOptions.isClearVideoWhereReplace {
            videoOutput?.flush()
        }
    }
...
@available(tvOS 14.0, *)
extension MoonMEPlayer: AVPictureInPictureSampleBufferPlaybackDelegate {
...
@available(macOS 12.0, iOS 15.0, tvOS 15.0, *)
extension MoonMEPlayer: AVPlaybackCoordinatorPlaybackControlDelegate {
...
extension MoonMEPlayer: DisplayLayerDelegate {
    public func change(displayLayer: AVSampleBufferDisplayLayer) {
        if #available(iOS 15.0, tvOS 15.0, macOS 12.0, *) {
            ...
            _pipController = MoonPictureInPictureController(contentSource: contentSource)
