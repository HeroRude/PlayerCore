...
//            imgConvertCtx = sws_getContext(width, height, self.format, width, height, dstFormat, SWS_FAST_BILINEAR, nil, nil, nil)
    ...
    private func transfer(frame: AVFrame) -> PixelBufferProtocol? {
        ...
        if let pbuf {
            ...
            pbuf.colorspace = MoonOptions.colorSpace(ycbcrMatrix: pbuf.yCbCrMatrix, transferFunction: pbuf.transferFunction)
        }
        return pbuf
    }
...
//    static let defaultValue = AudioDescriptor()
    ...
    init(sampleFormat: AVSampleFormat, sampleRate: Int32, channel: AVChannelLayout) {
        ...
        #else
        let channelCount = MoonOptions.outputNumberOfChannels(channelCount: AVAudioChannelCount(outChannel.nb_channels))
        #endif
    ...
    static func audioFormat(sampleFormat: AVSampleFormat, sampleRate: Int32, outChannel: inout AVChannelLayout, channelCount: AVAudioChannelCount) -> AVAudioFormat {
        ...
        UILog("[audio] out channelLayout: \(outChannel)")
        ...
        interleaved = MoonOptions.audioPlayerType == AudioRendererPlayer.self
        if !(MoonOptions.audioPlayerType == AudioRendererPlayer.self || MoonOptions.audioPlayerType == AudioUnitPlayer.self) {
            commonFormat = .pcmFormatFloat32
        }
    ...
    public func updateAudioFormat() {
        ...
        let channelCount = AVAudioChannelCount(2)
        #else
        let channelCount = MoonOptions.outputNumberOfChannels(channelCount: AVAudioChannelCount(channel.nb_channels))
        #endif
