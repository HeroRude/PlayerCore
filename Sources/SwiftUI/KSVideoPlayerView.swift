...
public struct KSVideoPlayerView: View {
    ...
    @StateObject
    private var playerCoordinator = MoonVideoPlayer.Coordinator()
    @Environment(\.dismiss)
    private var dismiss
    @FocusState
    ...
    public init(url: URL, options: MoonOptions, title: String? = nil, subtitleDataSouce: SubtitleDataSouce? = nil, onPlayerDisappear: ((MoonPlayerLayer?) -> Void)? = nil) {
    ...
    public var body: some View {
        ZStack {
            MoonVideoPlayer(coordinator: playerCoordinator, url: url, options: options)
...
//                    #endif
            ...
            if showDropDownMenu {
                VideoSettingView(config: playerCoordinator, subtitleModel: playerCoordinator.subtitleModel)
                    .frame(width: MoonOptions.sceneSize.width * 3 / 4)
...
struct VideoControllerView: View {
    @ObservedObject
    fileprivate var config: MoonVideoPlayer.Coordinator
    @State
...
struct VideoSettingView: View {
    ...
    @Environment(\.dismiss)
    private var dismiss
    init(config: MoonVideoPlayer.Coordinator, subtitleModel: SubtitleModel) {
        self.config = config
