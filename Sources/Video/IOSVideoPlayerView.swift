...
open class IOSVideoPlayerView: VideoPlayerView {
    ...
    open func updateUI(isFullScreen: Bool) {
        ...
        if isFullScreen {
            ...
            viewController.present(fullVC, animated: true) {
                MoonOptions.supportedInterfaceOrientations = fullVC.supportedInterfaceOrientations
            }
        } else {
            ...
            if let originalOrientations {
                MoonOptions.supportedInterfaceOrientations = originalOrientations
            }
    ...
    open func updateUI(isLandscape: Bool) {
        if isLandscape {
            topMaskView.isHidden = MoonOptions.topBarShowInCase == .none
        } else {
            topMaskView.isHidden = MoonOptions.topBarShowInCase != .always
        }
    ...
    override open func player(layer: MoonPlayerLayer, state: MoonPlayerState) {
    ...
    override open func player(layer: MoonPlayerLayer, currentTime: TimeInterval, totalTime: TimeInterval) {
    ...
    override open func panGestureChanged(velocity point: CGPoint, direction: KSPanDirection) {
        if direction == .vertical {
            if isVolume {
                if MoonOptions.enableVolumeGestures {
            ...
            } else if MoonOptions.enableBrightnessGestures {
                #if !os(xrOS)
    ...
    public func documentPicker(_: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
        if let url = urls.first {
            if url.isMovie || url.isAudio {
                set(url: url, options: MoonOptions())
            } else {
