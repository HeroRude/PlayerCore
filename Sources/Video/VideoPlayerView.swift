...
open class VideoPlayerView: PlayerView {
    ...
    override public var playerLayer: MoonPlayerLayer? {
        didSet {
...
//        }
    ...
    override open func set(url: URL, options: MoonOptions) {
    ...
    open func panGestureChanged(velocity point: CGPoint, direction: KSPanDirection) {
        if direction == .horizontal {
            if !MoonOptions.enablePlaytimeGestures {
                return
            }
            isSliderSliding = true
            if totalTime > 0 {
    ...
    open func panGestureEnded() {
        ...
        if scrollDirection == .horizontal, MoonOptions.enablePlaytimeGestures {
            hideSeekToView()
            isSliderSliding = false
